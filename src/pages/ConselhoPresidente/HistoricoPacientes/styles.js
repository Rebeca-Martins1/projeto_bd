...
export const PercentValue = styled.span`
  ...
  color: ${props => {
    ...
    if (percentual >= 20) return '#10b981';
    if (percentual >= 10) return '#d97706';
    if (percentual >= 5) return '#059669';
    return '#374151';
  }};
`;
...
export const DuracaoBadge = styled.span`
  ...
  background-color: ${props => {
    ...
    if (duracao >= 30) return '#fee2e2'; // > 30 dias
    if (duracao >= 15) return '#fef3c7'; // > 15 dias
    if (duracao >= 7) return '#d1fae5'; // > 7 dias
    return '#f3f4f6';
  }};

  color: ${props => {
    const duracao = props.duracao || 0;
    if (duracao >= 30) return '#dc2626';
    if (duracao >= 15) return '#92400e';
    if (duracao >= 7) return '#065f46';
    return '#374151';
  }};
`;
